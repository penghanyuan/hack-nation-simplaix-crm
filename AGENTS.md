...
`pnpm run dev` (or `pnpm dev`) launches the UI and agent together; `pnpm run dev:ui` and `pnpm run dev:agent` start only one side, with the latter invoking `uv run src/main.py`. Use `pnpm run dev:debug` for verbose logging, `pnpm run build` + `pnpm run start` for production checks, `pnpm run lint` for ESLint/TypeScript/Tailwind gates, and `pnpm run install:agent` whenever the Python dependencies change.
...
## Testing Guidelines
We expect Vitest + React Testing Library coverage for UI work, placing `*.test.tsx` next to the component and naming tests after the behavior (`handles empty CRM state`). Agent logic should be exercised with pytest files under `agent/tests`, executed via `uv run pytest`. Before opening a PR, run `pnpm run lint`, the relevant Vitest/pytest suites, and a manual smoke test (`pnpm run dev`, confirm the CopilotKit sidebar connects to port 8000).
...
## Agent Setup Notes
Secrets live only in `agent/.env` (`OPENAI_API_KEY=...`). During agent-focused work you may run `uv run src/agent.py` directly, but keep `pnpm run dev` running so the Next.js client continues to proxy requests to port 8000 and you validate the full stack.
